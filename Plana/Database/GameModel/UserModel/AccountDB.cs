...
namespace Plana.Database.GameModel
{
    public class AccountDBServer
    {
        ...
        public AccountDBServer(long publisherAccountId) : this()
        {
            ...
            this.ContentInfo = new()
            {
                ...
                TimeAttackDungeonDataInfo = new()
                {
                    SeasonId = 1,
                    SeasonBestRecord = 0
                },
                EliminateRaidDataInfo = new()
                {
                    SeasonId = 1,
                    BestRankingPoint = 0,
                    TotalRankingPoint = 0
                ...
                MultiFloorRaidDataInfo = new()
                {
                    SeasonId = 1
                },
            };
            this.BattleSummaries = new List<BattleSummaryDB>();
            this.RaidSummaries = new List<RaidSummaryDB>();
        }
    ...
    {
        public static AccountDBServer GetAccount(this SCHALEContext context, long accountId)
        {
            return context.Accounts.First(x => x.ServerId == accountId);
        }
        
        public static AccountDBServer? GetSingleAccount(this SCHALEContext context, long accountId)
        {
            return context.Accounts.SingleOrDefault(x => x.ServerId == accountId);
        }

        public static AccountDBServer GetAccountByPublisherId(this SCHALEContext context, long publisherId)
        {
            return context.Accounts.First(x => x.PublisherAccountId == publisherId);
        }

        public static AccountDBServer? GetSingleAccountByPublisherId(this SCHALEContext context, long publisherId)
        {
