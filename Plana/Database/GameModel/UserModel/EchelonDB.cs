...
namespace Plana.Database.GameModel
{
    ...
    {
        public static IQueryable<EchelonDBServer> GetAccountEchelons(this SCHALEContext context, long accountId)
        {
            return context.Echelons.Where(x => x.AccountServerId == accountId);
        }

        public static List<EchelonDBServer> AddEchelons(this SCHALEContext context, long accountId, params EchelonDBServer[] echelons)
        {
            if (echelons == null || echelons.Length == 0)
                return new List<EchelonDBServer>();
