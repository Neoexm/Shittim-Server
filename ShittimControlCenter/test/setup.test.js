...
// The body of installMitmproxy, which cannot be require()d - main.js pulls in electron. main.js is CRLF, so the closing
// brace has to be matched loosely or this quietly returns the rest of the file and every assertion below finds whatever
// it was looking for somewhere else.
...
// Inno Setup takes /VERYSILENT. Written --verysilent it is not an unknown-switch error, it is an unrecognised argument
// that Inno ignores, so the installer comes up as a normal GUI wizard behind the -Wait and the setup step hangs on a
...
// A silent installer that the user cancelled at the UAC prompt, or that failed part way, still leaves the step looking
// like it ran. Both halves are needed: the exit code, and then whether the thing it was supposed to install is there.
...
// cmd.exe still expands %NAME% inside the double quotes, so a tool under a folder with a percent sign in it comes back as
// "The filename, directory name, or volume label syntax is incorrect" and the SDK is reported missing while it is there.
...
// Deeper than the two levels the search walks, so it is deliberately not found rather than being found slowly across a
// Program Files that may hold a lot of directories.
