...
/// Unlike HasUnreadMail the gateway does not stamp this on every response: only the delivering response and the next Mail_Check carry it, and Mail_Check clears it even though the mail stays unread.
/// Clan attendance sets the bit on its own response instead and never calls MarkNewMail.
/// Mail predating the session never raises it, so keeping this in memory is fine - after a restart pending mail is exactly that pre-existing case.
/// </summary>
