...
// Attendance books ("stamp cards"), built to the wire model reconstructed from the captures. Account_Auth carries AttendanceBookRewards as the full definitions of every open book with a claimable day left today, and [] once everything is claimed - the pre-claim login at 04:12 shows both books, later logins the same game day show none - while AttendanceHistoryDBs is always the account's full history.
// Attendance_Reward repeats just the claimed book's definition plus its updated history row and carries no ParcelResultDB, because the reward arrives as mail (Type 1, UniqueId = book id, sender UI_MAILBOX_POST_SENDER_ARONA, comment UI_MAILBOX_ATTENDANCE_REWARD_MESSAGE_NORMAL, expiry send+7d); hence ServerNotification 12 on the response and bit 4 consumed by the next Mail_Check.
// History rows on the wire carry only ServerId, AttendanceBookUniqueId and AttendedDay.
public class AttendanceService
{
    ...
    // the books Account_Auth should carry: open books with an unclaimed day today.
    ...
    // returns the claimed book's definition plus the updated history row - exactly what the official response carries.
        ...
        SchaleDataContext db, AccountDBServer account, long bookUniqueId)
    {
        ...
        // Idempotent: a client that re-submits an already-claimed (or completed) book gets the current state back instead of an error popup - the claim loop can desync after menu backouts and re-logins.
        ...
        // A completed cyclical book starts a fresh pass. The official veteran's basic book (BookSize 10) shows only the current pass's days, so the dict resets rather than grows.
        ...
        // The reward is delivered as mail, never as an in-response parcel. Official's mail: Type = the book's MailType (1, Attendance), UniqueId = the book id, localisation keys as sender/comment, expiry exactly 7 days after send.
        ...
        // Attendance mail (unlike clan mail) leaves the NewMailArrived bit pending for the next Mail_Check - off1's first Mail_Check after the claims reports 12, the second 8.
        MailNotificationService.MarkNewMail(account.ServerId);

        return (BuildBook(excel), ToWire(history));
    }

    // The wire history carries only these three members - LastAttendedDay/LastAttendedDate stay server-side (official never emits them).
    ...
    // 0 when nothing is claimable; completed books cycle back to day 1.
    private static long NextDay(AttendanceExcelT excel, AttendanceHistoryDBServer? history)
    {
        ...
        if (attended < excel.BookSize)
            return attended + 1;
        // Book full: the basic book cycles forever (the official veteran's BookSize-10 history holds only the current pass's days); event books are one pass.
    ...
    private AttendanceBookReward BuildBook(AttendanceExcelT excel)
    {
        ...
        return new AttendanceBookReward
        {
            UniqueId = excel.Id,
            Type = excel.Type,
            // AccountType left at default: official never emits it on the wire.
            DisplayOrder = excel.DisplayOrder,
            AccountLevelLimit = excel.AccountLevelLimit,
            // no localisation key -> official emits '' rather than omitting the title.
            Title = excel.Title ?? "",
            TitleImagePath = excel.TitleImagePath ?? "",
            CountRule = excel.CountRule,
            CountReset = excel.CountReset,
            // Not present in the generated AttendanceExcel model; official sends 1 on the basic book and 3 on the Event20Days book, so it is inferred from the book type.
