...
public class EventContentSeasonLayoutTests
{
    private static EventContentSeasonExcelT Row(string label)
    {
        ...
        // ExcelTableService flips this the same way before reading either source.
    ...
    {
        // The two events official's Mission_List calls actually asked about. 10842 is the rerun of
        // 842; MissionHandler keys ClearedOrignalMissionIds off exactly these two members, so if
        // they decode wrong the response silently omits a key official sends (or vice versa).
        var rerun = Row("10842");
    ...
    {
        // Slot 15 is the field that was missing. Everything from here down was off by one slot, so
        // these four assertions are the actual alignment proof: three strings that must read as
        // "yyyy-MM-dd HH:mm:ss" in ascending order, and two that must read as asset paths.
        var row = Row("856");
    ...
    {
        // 38 slots is the widest vtable in the table, and this is the last of them. It reads as 0
        // under any layout that is short by a field, so it pins the total field count as well.
